...
from misc.encoder_MCB_H import _netE
from misc.encoder_Q import _netE as _netE_Q
import datetime
...
mcb_QI = CompactBilinearPooling(opt.ninp, (49 * img_feat_size), emb_size).cuda()

netE = _netE(opt.model, opt.ninp, opt.nhid, opt.nlayers, opt.dropout, emb_size)
netE_Q = _netE_Q(opt.model, opt.ninp, opt.nhid, opt.nlayers, opt.dropout)
...
if opt.model_path != '': # load the pre-trained model.
    ...
    netE_Q.load_state_dict(checkpoint['netE_Q'])
    netD.load_state_dict(checkpoint['netD'])

if opt.cuda: # ship to cuda, if has GPU
    netW.cuda(), netE.cuda(), netE_Q.cuda(),
...
def train(epoch):
    ...
    ques_hidden = netE_Q.init_hidden(opt.batchSize)
    ...
    while i < len(dataloader):
        ...
        image = image.view(batch_size, -1, img_feat_size)
        ...
        for rnd in range(10):
            ...
            ques_feat, ques_hidden = netE_Q(ques_emb, ques_hidden)
            qi_emb = mcb_QI(ques_feat,img_input.view(batch_size,-1))

            hist_hidden = repackage_hidden(hist_hidden, his_input.size(1))

            featD = netE(qi_emb, his_emb, hist_hidden, rnd+1)
