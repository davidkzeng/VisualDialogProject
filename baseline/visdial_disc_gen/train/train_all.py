    ...
    while i < len(dataloader):
        ...
        for rnd in range(10):
            ...
            if opt.update_LM:
                ...
                lm_loss = lm_loss / torch.sum(ans_target.data.gt(0)).float()
        ...
        i += 1
        err_lm_tmp = err_lm_tmp.cpu().numpy().tolist() if isinstance(err_lm_tmp, torch.Tensor) else err_lm_tmp
        err_d_tmp = err_d_tmp.cpu().numpy().tolist() if isinstance(err_d_tmp, torch.Tensor) else err_d_tmp
        err_g_tmp = err_g_tmp.cpu().numpy().tolist() if isinstance(err_g_tmp, torch.Tensor) else err_g_tmp
        err_d_fake_tmp = err_d_fake_tmp.cpu().numpy().tolist() if isinstance(err_d_fake_tmp, torch.Tensor) else err_d_fake_tmp
        err_g_fake_tmp = err_g_fake_tmp.cpu().numpy().tolist() if isinstance(err_g_fake_tmp, torch.Tensor) else err_g_fake_tmp
...
for epoch in range(opt.start_epoch+1, opt.niter):
    ...
    train_loss_lm = train_loss_lm.cpu().numpy().tolist() if isinstance(train_loss_lm, torch.Tensor) else train_loss_lm
    train_loss_d = train_loss_d.cpu().numpy().tolist() if isinstance(train_loss_d, torch.Tensor) else train_loss_d
    train_loss_g = train_loss_g.cpu().numpy().tolist() if isinstance(train_loss_g, torch.Tensor) else train_loss_g
